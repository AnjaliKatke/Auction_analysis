...
try{
	...
	String sql="SELECT name FROM player_profile WHERE player_id="+id+"";
	...
	{		
%>			       
			         <h3><span class="card-title deep-purple-text"><%=resultSet.getString("name")%></span></h3>
<%}
}	
			        catch(Exception e)
			        {
...
try{
	...
	int id=Integer.parseInt(player_id);
	System.out.println("Player id::"+player_id);
	String sql="SELECT * FROM player_statistics WHERE player_id="+id+"";
	Statement statement=connection.createStatement();
		//System.out.println(sql);
	//"SELECT idPlayers FROM pendu.players WHERE first= '" + tboxFirst.getText() + "'AND last='Man' AND pass=12345";
	ResultSet resultSet=statement.executeQuery(sql);
...
%>			
		        <tbody>
		          <tr>
	          		<td><%=resultSet.getString("record_type") %></td>
		            ...
		            <td><%=resultSet.getString("fiftys") %></td>
